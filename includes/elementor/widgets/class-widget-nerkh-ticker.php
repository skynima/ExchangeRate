...
class Nerkh_Chand_Widget_Ticker extends \Elementor\Widget_Base
{
    public function get_name()
    {
        return 'nerkhchand_ticker';
    }
    ...
    protected function register_controls()
    {
        ...
        $this->add_control('title', array('label' => 'عنوان', 'type' => \Elementor\Controls_Manager::TEXT, 'default' => 'تیکر نرخ'));
        ...
        $this->add_control('limit', array('label' => 'تعداد آیتم', 'type' => \Elementor\Controls_Manager::NUMBER, 'default' => 10, 'min' => 0));
    ...
    protected function render()
    {
        ...
        $shortcode = sprintf(
            '[exchange_rate source="%s" title="%s" symbols="%s" date="%s" limit="%d" view="ticker"]',
            esc_attr($s['source']),
            esc_attr($s['title']),
            esc_attr($s['symbols']),
            esc_attr($s['date']),
